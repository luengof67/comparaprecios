...
import '../services/hoja_pedido_service.dart';
...
class _ProductosScreenState extends State<ProductosScreen> {
  ...
  /// Genera la hoja PDF con los productos marcados (enLista).
  Future<void> _hojaPedido() async {
    setState(() => _generando = true);
    try {
      final productos = await db.productos().first;
      final precios = await db.precios().first;
      final proveedores = await db.proveedores().first;
      final comparativas = AnaliticaService.compararTodo(
          productos, precios, proveedores);
      final marcados = comparativas.where((c) => c.producto.enLista).length;
      if (marcados == 0) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text(
                  'No hay productos marcados en la lista. Márcalos en la pestaña Lista.')));
        }
        return;
      }
      await HojaPedidoService.generar(comparativas);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Error al generar la hoja: $e')));
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Hoja de pedido para imprimir',
            icon: _generando
                ? const SizedBox(
                    width: 18,
                    height: 18,
            ...
            onPressed: _generando ? null : _hojaPedido,
          ),
        ],
      ),
