...
class _BloqueProveedor extends StatelessWidget {
  ...
  Future<void> _enviarWhatsApp(BuildContext context) async {
    ...
    for (final c in marcados) {
      ...
      final unidad = c.producto.unidadBase.nombre;
      if (cant > 0) {
        // Si este proveedor tiene formato para el producto, pedir en cajas.
        OfertaProveedor? o;
        for (final x in c.ofertas) {
          if (x.proveedor.id == proveedor?.id) {
            o = x;
            break;
          }
        }
              ...
              children: [
                ...
                if (tieneFormato) ...[
                  SegmentedButton<bool>(
                    segments: [
                      ButtonSegment(
                          value: true, label: Text('En ${oferta.formato}s')),
                      ButtonSegment(value: false, label: Text('En $unidad')),
                    ],
                    selected: {enCajas},
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: enCajas && tieneFormato
                        ? 'Cuántas ${oferta.formato}s'
                        : 'Cantidad esta semana ($unidad)',
